...
class scanner_sink : public gr_block
{
	public:
		scanner_sink(osmosdr_source_c_sptr source, unsigned int vector_length, double centre_freq_1, double centre_freq_2, double bandwidth0, double bandwidth1, double bandwidth2,
				double step, unsigned int avg_size, double spread, double threshold, double ptime, float gain, float ppm) :
	...
	private:
		...
		void ProcessVector(float *input)
		{
			...
			if (m_avg_size == m_count){ //we've averaged over the number we intended to
				...
				PrintSignals(freqs, bands1, bands2, m_wait_count, m_time);
		...
		void PrintSignals(double *freqs, float *bands1, float *bands2, double wcount, double mtime)
		{
			...
			if (wcount == 0) {
			fprintf(stderr, "%02u:%02u:%02u: Scanning %f MHz - %f MHz (Offset: %f, RFGain: %f) ",
				hours, minutes, seconds, (m_centre_freq_1 - m_bandwidth0/2.0)/1000000.0, (m_centre_freq_1 + m_bandwidth0/2.0)/1000000.0, m_offset, m_gain);
			}
			else if (wcount == (mtime-1)) {
				fprintf(stderr,".Done.\n");
			}
			else {
				fprintf(stderr,".");
				...
				if (sig){ //we're already in a signal
					...
					if (diffs[i] < m_threshold){ //we're transitionning to the end
						...
						if (TrySignal(freqs[max], freqs[min])){
							printf("\n[+] %02u:%02u:%02u: Found signal: at %f MHz of width %f kHz, peak power %f dB (difference %f dB, Offset: %f, RFGain: %f)\n",
								hours, minutes, seconds, (freqs[max] + freqs[min]) / 2000000.0, (freqs[max] - freqs[min])/1000.0, bands1[peak], diffs[peak], m_offset, m_gain);
						}
					}
				}
				else {
...
scanner_sink_sptr make_scanner_sink(osmosdr_source_c_sptr source, unsigned int vector_length, double centre_freq_1, double centre_freq_2, double bandwidth0, double bandwidth1, double bandwidth2,
	double step, unsigned int avg_size, double spread, double threshold, double ptime, float gain, float ppm)
{
	return boost::shared_ptr<scanner_sink>(new scanner_sink(source, vector_length, centre_freq_1, centre_freq_2, bandwidth0, bandwidth1, bandwidth2, step, avg_size, spread, threshold, ptime, gain, ppm));
}
